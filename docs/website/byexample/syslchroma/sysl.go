...
// Sysl lexer.
var Sysl = lexers.Register(MustNewLexer(
	&Config{
		Name:      "Sysl",
		Aliases:   []string{"sysl"},
		Filenames: []string{"*.sysl"},
		MimeTypes: []string{"text/x-syslsrc"},
	},
	Rules{
		"root": {
			{`\n`, Text, nil},
			...
			{"(`)([^`]*)(`)", ByGroups(LiteralString, Using(TypeRemappingLexer(SyslTextTemplate, TypeMapping{{Other, LiteralString, nil}})), LiteralString), nil},
			{`"(\\\\|\\"|[^"])*"`, LiteralString, nil},
			{`(<<=|>>=|<<|>>|<=|>=|&\^=|&\^|\+=|-=|\*=|/=|%=|&=|\|=|&&|\|\||<-|\+\+|--|==|!=|:=|\.\.\.|[+\-*/%&])`, Operator, nil},
...
var SyslHTMLTemplate = lexers.Register(DelegatingLexer(h.HTML, MustNewLexer(
	&Config{
		Name:    "Sysl HTML Template",
		Aliases: []string{"sysl-html-template"},
	},
	syslTemplateRules,
)))

var SyslTextTemplate = lexers.Register(MustNewLexer(
	&Config{
		Name:    "Sysl Text Template",
		Aliases: []string{"sysl-text-template"},
	},
	syslTemplateRules,
))
