...
func TestLoadXsdFromTestFiles(t *testing.T) {
	...
	for _, f := range files {
		...
		if strings.EqualFold(parts[1], "xml") {
			filename := strings.Join(parts[:len(parts)-1], ".")
			t.Run(fmt.Sprintf("TestLoadXsdFromTestFiles - %s", filename), func(t *testing.T) {

				input, err := ioutil.ReadFile(path.Join(fileTestDir, filename+".xml"))
				require.NoError(t, err)
				...
				err = ioutil.WriteFile(path.Join(fileTestDir, filename+".sysl"), []byte(result), 0644)
				require.NoError(t, err)
