...
func TestHTML(t *testing.T) {
	...
	p.Direct = false
	fs := afero.NewOsFs()
	planutml := os.Getenv("SYSL_PLANTUML")
	filename := testDir + "multiple-app-datamodel.sysl"
	m, err := parse.NewParser().Parse(filename, fs)
	if err != nil {
		panic(err)
	...
	err = p.GenerateFromMap(outmap, fs)
	assert.Nil(t, err)
	err = diagrams.OutputPlantuml(p.Output, planutml, outmap["whatever.html"], fs)
	assert.Nil(t, err)
...
func TestLinkOutput(t *testing.T) {
	...
	err = p.GenerateFromMap(outmap, fs)
	assert.Nil(t, err)
	err = diagrams.OutputPlantuml(p.Output, os.Getenv("SYSL_PLANTUML"), outmap["whatever.html"], fs)
	assert.Nil(t, err)
	file, err := fs.Open(p.Output)
	assert.Nil(t, err)
	...
	expected := fmt.Sprintf(
		"%s/svg/UDgCaK5hmZ0OXk"+
			...
			"tfwBIv_Mx7u4M1h2hB2XdQYisftvnFzXC0"+
			"0F__jbPdCm00\n", planutml)
	assert.Equal(t, expected, planutml, string(link))
}
