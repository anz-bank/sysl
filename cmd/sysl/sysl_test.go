...
func TestCreateDBScriptValidSyslFile(t *testing.T) {
	...
	main2([]string{"sysl", "generatescript", "-t", "PetStore", "-o", "Relational-Model.sql", "-j", "Project",
		"-r", filepath.Join(testDir, "db_scripts/"),
		"-s", "dataForSqlScriptOrg.sysl"},
		fs, logger, main3)
	syslutil.AssertFsHasExactly(t, memFs, "/Relational-Model.sql")
}
...
func TestCreateDBScriptInValidSyslFile(t *testing.T) {
	...
	err := main2([]string{"sysl", "generatescript", "-t", "PetStore", "-o", "Relational-Model.sql", "-j", "Project",
		"-r", filepath.Join(testDir, "db_scripts/"),
		"-s", "invalid.sysl"},
		fs, logger, main3)
...
func TestCreateDBScriptNoAppSyslFile(t *testing.T) {
	...
	err := main2([]string{"sysl", "generatescript", "-t", "PetStore", "-o", "Relational-Model.sql", "-j", "Proj123",
		"-r", filepath.Join(testDir, "db_scripts/"),
		"-s", "dataForSqlScriptOrg.sysl"},
		fs, logger, main3)
...
func TestModDBScriptValidSyslFile(t *testing.T) {
	...
	main2([]string{"sysl", "generatescriptdelta", "-t", "PetStore", "-o", "Relational-Model.sql", "-j", "Project",
		"-r", filepath.Join(testDir, "db_scripts/"),
		"-s", "dataForSqlScriptOrg.sysl", "-n", "dataForSqlScriptModified.sysl"},
		fs, logger, main3)
	syslutil.AssertFsHasExactly(t, memFs, "/Relational-Model.sql")
}
...
func TestModDBScriptInValidOrgSyslFile(t *testing.T) {
	...
	err := main2([]string{"sysl", "generatescriptdelta", "-t", "PetStore", "-o", "Relational-Model.sql", "-j", "Project",
		"-r", filepath.Join(testDir, "db_scripts/"),
		"-s", "invalid.sysl", "-n", "dataForSqlScriptModified.sysl"},
		fs, logger, main3)
	assert.Equal(t, 2, err)
}
...
func TestModDBScriptInValidNewSyslFile(t *testing.T) {
	...
	err := main2([]string{"sysl", "generatescriptdelta", "-t", "PetStore", "-o", "Relational-Model.sql", "-j", "Project",
		"-r", filepath.Join(testDir, "db_scripts/"),
		"-s", "dataForSqlScriptOrg.sysl", "-n", "invalid.sysl"},
		fs, logger, main3)
	assert.Equal(t, 2, err)
}

func TestModDBScriptNoProjSyslFile(t *testing.T) {
	t.Parallel()
	logger, _ := test.NewNullLogger()
	_, fs := syslutil.WriteToMemOverlayFs("/")

	err := main2([]string{"sysl", "generatescriptdelta", "-t", "PetStore", "-o", "Relational-Model.sql", "-j", "Proj123",
		"-r", filepath.Join(testDir, "db_scripts/"),
		"-s", "dataForSqlScriptOrg.sysl", "-n", "dataForSqlScriptModified.sysl"},
		fs, logger, main3)
	assert.Equal(t, 1, err)
...
func TestModDBScriptNewAppSyslFile(t *testing.T) {
	...
	err := main2([]string{"sysl", "generatescriptdelta", "-t", "PetStore", "-o", "Relational-Model.sql", "-j", "Project",
		"-r", filepath.Join(testDir, "db_scripts/"),
		"-s", "dataForSqlScriptOrg.sysl", "-n", "dataForSqlScriptModifiedTwoApps.sysl"},
		fs, logger, main3)
