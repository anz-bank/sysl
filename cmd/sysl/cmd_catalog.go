...
func (p *catalogCmd) Configure(app *kingpin.Application) *kingpin.CmdClause {
	...
	cmd.Flag("projectTemplate", "projectTemplate filname to use").StringVar(&p.projectTemplate)
	cmd.Flag("packageTemplate", "packageTemplate filname to use").StringVar(&p.packageTemplate)
...
func (p *catalogCmd) Execute(args cmdutils.ExecuteArgs) error {
	...
	if p.projectTemplate != "" {
		f1, err = args.Filesystem.Open(p.projectTemplate)
		if err != nil {
			log.Fatal(err)
		}
	}
	if p.packageTemplate != "" {
		f2, err = args.Filesystem.Open(p.packageTemplate)
		if err != nil {
			log.Fatal(err)
		}
	}
