...
--------------------------------------------- */
`
const pr_relationArrow = `}--`
const pr_tupleArrow = `*--`
const pr_entityLessThanArrow = `<< (`
const pr_entityGreaterThanArrow = `) >>`
const pr_classString = `class`
...
func (v *DatabaseScriptView) GenerateDatabaseScriptModify(dataParam *DatabaseScriptModifyParam) string {
	...
	for _, tableDetail := range tableDetails {
		if strings.EqualFold(tableDetail.action, "ADD") {
			v.writeCreateSqlForRelation(tableDetail.name, tableDetail.table.GetRelation(), relationshipMap, visitedAttributes)
		} else if strings.EqualFold(tableDetail.action, "RETAIN") {
			v.writeModifySqlForRelation(tableDetail.name, tableDetail.table.GetRelation(), tableDetail.tableOld.GetRelation(), relationshipMap, visitedAttributes)
		} else if strings.EqualFold(tableDetail.action, "DELETE") {
...
func (v *DatabaseScriptView) writeModifySqlForRelation(
	...
	for _, attrNameOld := range attrNamesListOld {
		...
		if attrTypeNew == nil {
			...
			dropColumnQueries = dropColumnQueries + fmt.Sprintf("ALTER TABLE %s DROP COLUMN %s;\n", entityName, attrNameOld)
		}
	}
...
func (v *DatabaseScriptView) writeModifySqlForAColumn(attrTypeOld, attrTypeNew *proto.Type, entityName, attrName string, primaryKeys *[]string, visitedAttributes map[string]string) (bool, bool) {
	...
	if typeRefNew != nil {
		...
		if typeRefOld == nil {
			...
			if !strings.EqualFold(typeRefNew.GetRef().Path[0]+"."+typeRefNew.GetRef().Path[1], typeRefOld.GetRef().Path[0]+"."+typeRefOld.GetRef().Path[1]) {
		...
		if !strings.EqualFold(datatype, datatypeOld) {
			...
			if isAutoIncrementNew != isAutoIncrementOld {
