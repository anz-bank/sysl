...
if [[ $COMMAND = "prepare" ]]; then
    ...
    echo "__version__ = '$VERSION'" > 'pkg/sysl/__version__.py' || fatal "Could not override pkg/sysl/__version__.py"
    echo "syslVersion=$VERSION" > 'pkg/libs/java/gradle.properties' || fatal "Could not override pkg/sysl/__version__.py"
    ...
    PY_VERSION=$(sed -n "s/__version__ = '\\([^ ]*\\)'/\\1/p" pkg/sysl/__version__.py)
