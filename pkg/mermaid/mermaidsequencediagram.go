...
func generateMermaidSequenceDiagramHelper(m *sysl.Module, appname string, epname string,
	...
	result += printSequenceDiagramStatements(m, statements, appname, previousapp, indent, thestart)
	return result, nil
}
...
func printSequenceDiagramStatements(m *sysl.Module, statements []*sysl.Statement, appname string,
	...
	for _, statement := range statements {
		...
		case *sysl.Statement_Group:
			...
			result += printSequenceDiagramStatements(m, c.Group.Stmt, appname, previousapp, indent+1, thestart)
		...
		case *sysl.Statement_Cond:
			...
			result += printSequenceDiagramStatements(m, c.Cond.Stmt, appname, previousapp, indent+1, thestart)
		...
		case *sysl.Statement_Foreach:
			...
			result += printSequenceDiagramStatements(m, c.Foreach.Stmt, appname, previousapp, indent+1, thestart)
			result += fmt.Sprintf("%send\n", addIndent(indent))
		case *sysl.Statement_Loop:
			result += fmt.Sprintf("%sloop %s\n", addIndent(indent), c.Loop.Criterion)
			result += printSequenceDiagramStatements(m, c.Loop.Stmt, appname, previousapp, indent+1, thestart)
			result += fmt.Sprintf("%send\n", addIndent(indent))
		case *sysl.Statement_LoopN:
			result += fmt.Sprintf("%sloop %d times\n", addIndent(indent), c.LoopN.Count)
			result += printSequenceDiagramStatements(m, c.LoopN.Stmt, appname, previousapp, indent+1, thestart)
			result += fmt.Sprintf("%send\n", addIndent(indent))
		default:
