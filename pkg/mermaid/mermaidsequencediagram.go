...
func GenerateMermaidSequenceDiagram(m *sysl.Module, appname string, epname string) (string, error) {
	var sp []sequencepair
	return generateMermaidSequenceDiagramHelper(m, appname, epname, "...", 1, &sp, true)
}
		...
		case *sysl.Statement_Call:
			...
			if !sequencenpairscontain(*sequencepairs, pair) {
				*sequencepairs = append(*sequencepairs, pair)
				...
				previous := appname
				out, err := generateMermaidSequenceDiagramHelper(m, nextapp, nextep, previous, indent, sequencepairs, false)
				if err != nil {
...
func sequencenpairscontain(s []sequencepair, sp sequencepair) bool {
	for _, a := range s {
		if a == sp {
			return true
		}
