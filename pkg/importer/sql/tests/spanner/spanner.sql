...
ALTER TABLE Account2 ADD ROW DELETION POLICY (OLDER_THAN(created, INTERVAL 30 DAY));
ALTER TABLE Account2 DROP ROW DELETION POLICY;
ALTER TABLE Account2 REPLACE ROW DELETION POLICY (OLDER_THAN(created, INTERVAL 60 DAY));
...
CREATE FUNCTION mysql.MAKEDATE(y INT64, d INT64)
RETURNS DATE
AS (
  SAFE.IF(
    d > 0,
...
CREATE OR REPLACE FUNCTION mysql.SECOND(ts TIMESTAMP) RETURNS INT64 AS (EXTRACT(SECOND FROM ts));
