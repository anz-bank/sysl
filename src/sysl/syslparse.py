...
class SyntaxError(Exception):
  ...
  def __init__(self, parser, fmt, *args, **kwargs):
    ...
    super(Exception, self).__init__(fmt.format(*args, **kwargs), source_context)
  ...
  def _call_tail(self, single=False):
    while self.eat(self.TAIL_PATTERN):
      ...
      if op == '[':
        ...
        arg = self.pop()
        [lhs] = self.push(sysl_pb2.Expr(call=sysl_pb2.Expr.Call(func=arg.name)))
      ...
      # -> (...
        ...
        if out_type:
          ...
          target_type.type_ref.ref.appname.part.extend(target_path[:1])
        ...
        while self.eat(r','):
          if not self._expr():
            raise RuntimeError('missing expr')
          args.append(self.pop())
          desc.append(self.eat(r'(asc|desc)\b') and self.pop() == 'desc')
...
class Parser(object):
    ...
    try:
      ...
      for (line_no, line) in enumerate(src):
        if rex.match(ur'·\#', line):
          continue
        (indent, pipe, text) = rex.match(ur'^(·)(\|?)(.*?)·$', line).groups()
        if pipe:
          assert text[:1] in (' ', ''), (
        ...
        if line.endswith(':') and not line.startswith('|'):
          ...
          if line.startswith('@'):
            ...
            if not all(c.startswith('|') for (_, _, c, _) in children):
              raise RuntimeError("@attr =: children must be multiline string")
            no_space = [
            ...
            cc = json.dumps(' '.join(c[2:] for (_, _, c, _) in children))
            line += cc
            children = []
          ...
          children = []
          if i + 1 < len(lines):
            (line_no2, end_line_no2, line2, children2) = lines[i + 1]
  ...
  def _parse_type(self, app, line_no, name, typespec, type_):
    ...
    if match(ur'''
      ...
      primitive = ('_' not in ident) and getattr(type_, ident.upper(), None)
        ...
        if attrname:
          ...
          if grandchildren and not typespec:
            ...
            if typespec:
              if max_repeat > 1:
                attrtype = attrtype.list.type
              self._parse_type(app, line_no, name, typespec, attrtype)
          ...
          if grandchildren:
            self._parse_type_decl(app, keyword, attrname, grandchildren)
          else:
            raise NotImplemented('name missing (and no field injection yet)')
    ...
    for (line_no, _, line, grandchildren) in children:
      ...
      if match(ur'^(.*?)·(->(?:·<·(set\s+of\b·)?([\w\.]*)·>)?)·\((\w*)$'):
        ...
        self._parse_transform(
          app, expr_field, expr, bool(setof), out_type, scopevar or '.',
          grandchildren)
      ...
      if match(r'^((?:[^=]|=(?!>))+)\s*=>\s*(.*?)$'):
        ...
        for control in rex.split(r'\s*,\s*', controls):
          if ifexpr == 'true':
            ...
            callexpr.func = 'bool'
            self._parse_expr(control, grandchildren, callexpr.arg.add())
          else:
  ...
  def _parse_app(self, module, app, name, long_name, attrs, endpoints):
    ...
    for (_, _, endpoint_line, stmts) in endpoints:
      ...
      if match(r'^@(.*)'):
        ...
        [name] = match.groups
        self._parse_app(module, app.mixin2.add(), name, None, None, stmts)
        stmts = []
      elif match(r'^!wrap\s+(.*)$'):
        [name] = match.groups
