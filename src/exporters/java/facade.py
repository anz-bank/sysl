    ...
    for (tname, ft, t) in syslx.wrapped_facade_types(context):
      if t.HasField('relation'):
        ...
        with java.Method(w, '\npublic', tname + 'Facade', 'get' + tname):
          w('return {}Facade;', java.safe(tname[:1].lower() + tname[1:]))
          ...
          if param_defs or required:
            ...
            for perm in range(2**len(keys)):
              ...
              with java.Class(w, bname, context.write_file):
                done = [k for (i, k) in enumerate(keys) if 2**i & perm]
                ...
                for fname in remaining:
                  ...
                  method = java.CamelCase(jfname)
                  (java_type, type_info, _) = datamodel.typeref(f, context.module)
                  next_bname = 'Builder{}'.format(perm | 2**keyindices[fname])
                  w()
                  ...
                  for (i, (r, rtype)) in enumerate(required):
                    ...
                    if jfname in fkeys:
                      ...
                      with java.Method(w, 'public', bname, 'with' + method,
                        ...
                        w('this.{0} = {0};', java.mixedCase(fk_type))
                        w('_pending &= ~{};', hex(2**i))
                        w('return this;')
                    else:
                      with java.Method(w, 'public', bname, 'with' + method,
                    ...
                    if required:
                      ...
                      for (r, rtype) in required:
                        if jfname in fkeys:
                          ...
                          w('result.set{}({});', java.CamelCase(r), r)
                      w('return result;')
                    else:
