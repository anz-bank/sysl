...
def codeForType(t, scope):
  ...
  if which_type == 'primitive':
    ...
    if ref.HasField('appname'):
      ...
      assert app, 'Appname {} not found'.format('.'.join(ref.appname.part))
      package = syslx.View(app.attrs)['package'].s
      assert package, ref
      is_anon = ref.path and re.match(r'AnonType_\d+__$', '.'.join(ref.path))
      return '{}{}{}'.format(
        package,
...
def codeForExpr(w_, expr, scope, module, let=None):
  ...
  def E(parent_pc, expr, let=None):
    ...
    if which_expr == 'name':
      ...
      if func == 'bool':
        ...
        assert len(args) == 2
        return (
          protect(pc, 'DateTimeFormat.forPattern({1}).print(new DateTime({0}))'.format(*args)),
          sysl_pb2.Type(primitive=sysl_pb2.Type.STRING))
      ...
      (elems, types) = zip(*[E(-1, e) for e in exprs]) if exprs else [(), ()]
  ...
  def ifelseExprToJava(protect, ifelse):
    ...
    if ifelse.nullsafe:
      (cond_init_code, cond_code, _) = tmpForExpr(coerce_to_bool(ifelse.cond))
      return (
    ...
    return (
      protect('?:', '{} \v? {} \v: \v{}'.format(cond, if_true, if_false)),
      result_type)

  def whereToJava(protect, binexpr):
    pc = PRECEDENCE['.']
    (arg, arg_type) = E('.', binexpr.lhs)
    assert arg_type.WhichOneof('type') == 'set', arg_type.WhichOneof('type')
...
#        if which_pred_type == 'set':
    ...
    def toJava(protect, binexpr):
      ...
      def attr_names(arg):
        ...
        with inner_java_scope():
          ...
          attr_defs = getattr(resolved_arg_type, which_type).attr_defs
  ...
  def binexprToJava(prec, fmt, out_type=None):
    ...
    def b2j(protect, binexpr):
      ...
      return (
        protect(prec or PRECEDENCE[(2, binexpr.op)], fmt.format(lhs, rhs)),
          ...
          for stmt in transform.stmt:
            ...
            if which_stmt == 'assign':
              ...
              if assign.expr.WhichOneof('expr'):
                (code, _) = codeForExpr(w, assign.expr, transform_scope, module)

                w('out.{}set{}\037 ({});',
                  set_prefix, CamelCase(assign.name), code)
              else:
                w('// out.{}set{}(...);', set_prefix, CamelCase(assign.name))
              ...
              w('{};', codeForExpr(w, stmt.inject, transform_scope, module)[0])

            else:
              raise RuntimeError('Unexpected stmt type: ' + which_stmt)

        if is_view:
          is_table = scope.resolve(out_type)[1].WhichOneof('type') == 'relation'
          code = (
