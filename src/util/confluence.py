    ...
    def __call__(self, url, *args, **kwargs):
      ...
      if self.name not in ('get', 'head') and not self.lazy_session._auth_added:
        ...
        if user is None:
          ...
          env_hint = ' (=USER env var; set {} to override)'.format(USER_ENV)
...
def _confluence_to_http(confluence_url):
  ...
  (space, title, tail, att, query) = re.match(r'confluence://([^/]+)/(.+?)(/(?:@(.*?)|(?:[^@].*?)))?(\?.*)?$', confluence_url).groups()

  content(None, query={'space':space, 'title':title})
  response = LAZY_SESSION.get(content(None, query={'space':space, 'title':title}), verify=VERIFY_CERTS)
  ...
  for result in root['results']:
    child_id = result['id']
    response = LAZY_SESSION.get(content(child_id, 'child', 'attachment'), verify=VERIFY_CERTS)
    response.raise_for_status()
    root = response.json()
    for result in root['results']:
      if result['title'] == att:
...
def get_storage(confluence_url):
  response = LAZY_SESSION.get(_confluence_to_http(confluence_url) + '?expand=body.storage,ancestors,version', verify=VERIFY_CERTS)
  response.raise_for_status()
  root = response.json()
  body = root['body']
